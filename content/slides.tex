...
%         \end{minted}
    ...
    As an example, allowed is the \mintinline{c++}{<cmath>} header since there is no pure C++ alternative. 
    Not allowed are, e.g., \mintinline{c++}{printf} (use \mintinline{c++}{std::cout} or \mintinline{c++}{std::format} instead) or the C string functions operating on \mintinline{c++}{const char *} (use \mintinline{c++}{std::string} instead). 
    %For Java, everything in \texttt{java.*} may be used.
...
\begin{algorithmcl}
    \begin{algorithmic}[1]
        \State{Adjust initial velocities: $\vec{u} = \dfrac{\sum_j m_j \vec{v}_j}{\sum_j m_j}$;\quad$\vec{v}_i = \vec{v}_i - \vec{u}$}
...
% \end{center}
\begin{center}
    \mintinline{text}{./simulate --file data.csv --dt 2.5 --t_end 100.0 --vs 5}
\end{center}
...
% \end{center}
\begin{center}
    \mintinline{text}{./simulate. --vs 10 --dt 20 --t_end 4000 --file data2.csv}
\end{center}
...
\begin{frame}[fragile, label={phase1_anforderungen}]
  ...
  \begin{center}
    ...
    \mintinline{text}{./simulate --file 4000.csv --dt 300 --t_end 6000000 --vs 200} \\[.75em]
    \begin{figure}
...
\begin{frame}[fragile]
    ...
    \item Use an IDE (Visual Studio Code, CLion, Eclipse, etc.) and its features such as profilers, debuggers, auto-formatting, linters, etc.
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
    \item %If you are using \texttt{C++}: 
    Objects allocated with \mintinline{c++}{new} are \textbf{not} automatically released (better: use \mintinline{c++}{std::unique_ptr/std::shared_ptr} if necessary)!
    \item %If you are using \texttt{C++} 
